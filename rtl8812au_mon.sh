...
change_network_interface_name() {
    ...
    read -p "[?] New network interface name[rtl8812au-mon]: " network_interface_name
    if [[ $network_interface_name == '' ]]; then
        sudo ip l s $network_interface down
        sudo ip link set $network_interface name rtl8812au-mon
        sudo ip l s rtl8812au-mon up

        network_interface=rtl8812au-mon
...
while true; do
    ...
    echo -e "\n"
    read -p "Do you want to install the drivers for chipset rtl8812au? [N/y] " install_drivers_option
